...
function SelectUser(MODULE_ID,TO_ID, TO_NAME, MANAGE_FLAG,FORM_NAME,USE_UID)
{
  ...
  LoadDialogWindow(URL,self,loc_x, loc_y, 400, 350);//锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷选锟剿达拷锟节的匡拷群透叨锟?
}
function SelectUserNew(MODULE_ID,TO_ID, TO_NAME, MANAGE_FLAG,FORM_NAME,USE_UID,SELE_DEPT_ID)
{
    URL="/module/user_select_new?MODULE_ID="+MODULE_ID+"&TO_ID="+TO_ID+"&TO_NAME="+TO_NAME+"&MANAGE_FLAG="+MANAGE_FLAG+"&FORM_NAME="+FORM_NAME+"&USE_UID="+USE_UID+"&SELE_DEPT_ID="+SELE_DEPT_ID;
    loc_y=loc_x=200;
    if(is_ie)
    {
        loc_x=document.body.scrollLeft+event.clientX-100;
...
function SelectUserExternal(MODULE_ID, TO_ID, TO_NAME, MANAGE_FLAG, FORM_NAME)
{
  ...
  //window.open(URL);
  LoadDialogWindow(URL,self,loc_x, loc_y, 220, 350);//锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷选锟剿达拷锟节的匡拷群透叨锟?
}


function ClearDept(DEPT_ID, DEPT_NAME){
    $('#'+DEPT_ID).val('');
    $('#'+DEPT_NAME).val('');
}

function SelectDeptSingleCompany(MODULE_ID,TO_ID, TO_NAME, PRIV_OP,FORM_NAME,COMPANY_NO)
{
    URL="/module/dept_select_single_company?MODULE_ID="+MODULE_ID+"&TO_ID="+TO_ID+"&TO_NAME="+TO_NAME+"&PRIV_OP="+PRIV_OP+"&FORM_NAME="+FORM_NAME+"&COMPANY_NO="+COMPANY_NO;
    loc_y=loc_x=200;
    if(is_ie)
    {
        loc_x=document.body.scrollLeft+event.clientX-100;
